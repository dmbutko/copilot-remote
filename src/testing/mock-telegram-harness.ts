...
export class MockTelegramHarness implements Client {
	...
	async editMessage(chatId: string, msgId: number, text: string): Promise<void> {
		...
		this.log('edit', msgId, this.formatMessage(chatId, text, existing?.threadId ? { threadId: existing.threadId } : undefined));
	}
	...
	async editButtons(chatId: string, msgId: number, text: string, buttons: Button[][]): Promise<void> {
		...
		this.log('edit-buttons', msgId, this.formatMessage(chatId, text, existing?.threadId ? { threadId: existing.threadId } : undefined));
