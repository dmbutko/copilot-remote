...
export class TelegramBridge {
  ...
  setMessageHandler(handler: typeof this.onMessage): void { this.onMessage = handler; }
  setCallbackHandler(handler: typeof this.onCallback): void { this.onCallback = handler; }
  setReactionHandler(handler: typeof this.onReaction): void { this.onReaction = handler; }
      ...
      try {
        ...
        for (const update of updates) {
          ...
          if (update.callback_query) {
            ...
            await this.api('answerCallbackQuery', { callback_query_id: cb.id }).catch(() => { /* ignore */ });
          }
  ...
  async sendMessageWithButtons(chatId: string | number, text: string, buttons: { text: string; data: string }[][]): Promise<number | null> {
    const markup = { inline_keyboard: buttons.map((row) => row.map((btn) => ({ text: btn.text, callback_data: btn.data }))) };
  ...
  async sendTyping(chatId: string | number): Promise<void> {
    await this.api('sendChatAction', { chat_id: chatId, action: 'typing' }).catch(() => { /* ignore */ });
  }

  async setReaction(chatId: string | number, messageId: number, emoji: string): Promise<void> {
    const safe = toTelegramReaction(emoji);
    await this.api('setMessageReaction', { chat_id: chatId, message_id: messageId, reaction: [{ type: 'emoji', emoji: safe }] }).catch(() => { /* ignore */ });
  }

  async removeReaction(chatId: string | number, messageId: number): Promise<void> {
    await this.api('setMessageReaction', { chat_id: chatId, message_id: messageId, reaction: [] }).catch(() => { /* ignore */ });
  }

  // ── Bot commands ──

  async setMyCommands(commands: { command: string; description: string }[]): Promise<void> {
    await this.api('setMyCommands', { commands }).catch(() => { /* ignore */ });
  }
  ...
  private async getUpdates(): Promise<any[]> {
    const res = await this.api('getUpdates', { offset: this.offset, timeout: 30, allowed_updates: ['message', 'callback_query', 'message_reaction'] });
    return res.result ?? [];
  }
