...
describe('SessionRegistry invalidate', () => {
  let prevHome: string | undefined;
  let tmpToCleanup: string | null = null;
  ...
  it('invalidateChat suspends the session and clears any in-flight start', async () => {
    let resolveStart!: () => void;
    const startDeferred = new Promise<void>((r) => {
      resolveStart = r;
    });
    ...
    resolveStart();
    await p;
    assert.ok(sessions.has('chat-1'));

    reg.invalidateChat('chat-1', 'test');
  ...
  it('invalidateAll clears every live session', async () => {
    const factory = () => makeFakeSession({ startDeferred: Promise.resolve() });
    const { reg, sessions, tmpHome } = makeRegistry({ sessionFactory: factory });
    tmpToCleanup = tmpHome;

    await reg.getSession('chat-1');
    await reg.getSession('chat-2');
    assert.equal(sessions.size, 2);

    reg.invalidateAll('test-all');
