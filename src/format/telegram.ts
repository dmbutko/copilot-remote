import { chunkMarkdownIR, markdownToIR, type MarkdownLinkSpan, type MarkdownIR } from './ir.js';
...
/**
 * File extensions that share TLDs — wrapped in <code> to prevent Telegram
 * from generating spurious domain registrar previews.
 */
...
function sliceLinkSpansLocal(links: MarkdownIR['links'], start: number, end: number): MarkdownIR['links'] {
...
function splitMarkdownIRPreserveWhitespace(ir: MarkdownIR, limit: number): MarkdownIR[] {
  ...
  while (cursor < ir.text.length) {
    ...
    chunks.push({
      ...
      links: sliceLinkSpansLocal(ir.links, cursor, end),
    });
    cursor = end;
  }
  return chunks;
